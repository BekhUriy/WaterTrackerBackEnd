    ...
    try {
        ...
        if (existingUser) {
            return res.status(409)
                .json({message: "This email is already in use. Please check email or log in"});
        }
        ...
        //         <button><a href="http://${base}/api/auth/verify/${verificationToken}">Click to
        // Verify Email</a></button>. <p>If you did not register for Water Tracker, we recommend
        // you to ignore this letter.</p> <p>Keep yourself healthy!</p> <p>Best wishes,</p>
    ...
    try {
        ...
        if (passwordCompare) {
            ...
            await User.updateOne({_id: id}, {
                verificationToken,
                tempPasswordStorage: normalizedNewPassword,
                updatedAt: Date.now(),
            });
...
export const verifyPasswordChange = async (req, res) => {
    try {
        ...
        await User.updateOne({verificationToken},
            {password: hashedPassword, verificationToken: null, tempPasswordStorage: ""});
        ...
        if (passwordCompare) {
            ...
            if (validate.error) {
                return res.status(401)
                    .json({message: "validationn error", error: validate.error.message});
            }
            ...
            //     to: email,
            //     subject: "Change password",
            //     html: `<p>Hello! You received this email because you recently changed your
            // password.</p> <p>We are happy that you continue to use Water Tracker and hope that
            // you have a great time while using our app.</p> <p>Keep yourself healthy!</p> <p>Best
            // wishes,</p> <p>Water Tracker Team</p>` }; await sendEmail(emailOptions);
